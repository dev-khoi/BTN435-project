### Overview
## Web-based dashboard
<img width="1070" height="518" alt="Screenshot 2026-09-03 101711" src="https://github.com/user-attachments/assets/a98dc4d4-2c8d-420b-9123-1b39a6dc3163" />
...
The web controller has 3 running parts:

1. C++ TCP server (default: `127.0.0.1:8080`)
2. Node bridge API (default: `http://localhost:3000`)
3. Browser dashboard (`web/index.html`)

Recommended startup flow (use separate terminals):

1) Start the C++ server

- If you already have the built executable in project root:

```bash
./server_all_devices_new.exe
```

- Or run with a custom port:

```bash
./server_all_devices_new.exe 8080
```

2) Start the HTTP-to-TCP bridge
...
- Use buttons for:
  - Light: ON/OFF/STATUS
  - Thermostat: SET/STATUS
  - Camera: STATUS
  - Network: ROUTING TABLE / ARP TABLE

### Bridge Environment Variables (Optional)

- `BRIDGE_PORT` (default `3000`)
- `TCP_HOST` (default `127.0.0.1`)
- `TCP_PORT` (default `8080`)

Example:

```bash
BRIDGE_PORT=3001 TCP_HOST=127.0.0.1 TCP_PORT=8080 npm start
```
...
## Web Control Panel (No C++ Backend Changes)

This project now includes a browser dashboard and a local HTTP-to-TCP bridge that works with the existing C++ protocol.

Architecture:

Browser (HTML/CSS/JS) -> Node/Express bridge (`POST /api/command`) -> TCP socket -> existing C++ server
...
1. Start the C++ server on port `8080`.
2. Start the bridge:
...
3. Open `web/index.html` in your browser (or serve the `web/` folder with any static file server).
4. Use the dashboard buttons to send commands.
